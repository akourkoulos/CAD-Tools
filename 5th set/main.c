...
int convertInformation(int testout[],struct Lgate gNOT, struct Lgate gNOR, struct Lgate gNAND, struct Lgate gXOR,int* node,int netlist[][3],int inp[],int testinp[],int logicgate[],int netlistOut[],int FinalNet[][3],int FinalNetcount,int transistorNet[],int inpNet[],int nodeNet[],int matchGate[][2],int testInpNet[],int OutOfNet[]){
    int flag=1;
    int IsInList[500] ={};
    int numOfInp=0;
    int offset=0;
    ...
    int i=0;     //////////////// sets the outputs of the new netlist
     while(testout[i]!=0){
       OutOfNet[i]= netlistOut[testout[i]];
       i++;
    }

         ////////////// sets the test_Inputs of the new netlist
    i=0;
    while(testinp[i]!=0){
        int j=0;
        while (inp[j] !=0){
...
void findPossibleBreak(int possibleBreak[], int FinalNet[][3]){
    ...
    selectionSort(possibleBreak,sizeof(possibleBreak));
...
struct Graph* createDAG(int FinalNet[][3],int transistor[] ){
    ...
    while(FinalNet[i][0]!=0){////////////////////////////// create the eadges between all nodes
                int point=0;
                while(graph->head[FinalNet[i][2]-1]->next[point]!=NULL) {point++;}
                graph->head[FinalNet[i][2]-1]->next[point]=graph->head[FinalNet[i][0]-1];
                graph->head[FinalNet[i][2]-1]->rededgeVal[point]=transistor[i];
                if(transistor[i]==PMOS){
                    point=0;
                    while(graph->head[FinalNet[i][2]-1]->nextBlue[point]!=NULL) {point++;}
                    graph->head[FinalNet[i][2]-1]->nextBlue[point]=graph->head[FinalNet[i][1]-1];
                    graph->head[FinalNet[i][2]-1]->blueedgeVal[point]=transistor[i];
                }
                int point1=0;
                while(graph->head[FinalNet[i][1]-1]->next[point1]!=NULL) {point1++;}
                graph->head[FinalNet[i][1]-1]->next[point1]=graph->head[FinalNet[i][0]-1];
                graph->head[FinalNet[i][1]-1]->rededgeVal[point1]=transistor[i];
                if(transistor[i]==NMOS){
                    point1=0;
                    while(graph->head[FinalNet[i][1]-1]->nextBlue[point1]!=NULL) {point1++;}
                    graph->head[FinalNet[i][1]-1]->nextBlue[point1]=graph->head[FinalNet[i][2]-1];
                    graph->head[FinalNet[i][1]-1]->blueedgeVal[point1]=transistor[i];
                }
                i++;
    }//////////////////////////////////////////////////////////////////////////////////////////////
...
struct Graph* createGraph(int FinalNet[][3],int breakNode ){
    ...
    for(i=0;i<max;i++){   //creates all nodes and put them in the graph
        ...
        newNode->next[j] = NULL;
        newNode->rededgeVal[j] = 0;
        newNode->nextBlue[j] = NULL;
        newNode->blueedgeVal[j] = 0;
        }
        newNode->nodenumber=i+1;
        graph->head[i]=newNode;
    }
...
char* createFileOfGates( int FinalNet[][3] ,int gatesOfBreakNode[] ,int inpNet[] ,int OutOfNet[] ,char* Lib ){
   FILE *fptr1;
   fptr1 = fopen("FileOfGates.txt","w");//open file for write
   if(fptr1 == NULL)
       {
          printf("Error!");
          exit(1);
       }
    fprintf(fptr1,"## LIBRARY\n");
    fprintf(fptr1,"%s\n",Lib);
    fprintf(fptr1,"## RAILS\n");
    fprintf(fptr1,"## INPUTS\n");
    fprintf(fptr1,"%d",inpNet[0]);// write first input in the file
    int i=1;
    while(inpNet[i]!=0){  // write inputs in the file
        fprintf(fptr1,",%d",inpNet[i]);
        i++;
    }
    fprintf(fptr1,"\n## OUTPUTS\n");
    fprintf(fptr1,"%d",OutOfNet[0]);// write first output in the file
    i=1;
    while(OutOfNet[i]!=0){  // write outputs in file
        fprintf(fptr1,",%d",OutOfNet[i]);
        i++;
    }
    fprintf(fptr1,"\n## NETLIST\n");
    i=0;
    while(FinalNet[i][0]!=0){// write the netlist in file
        if(gatesOfBreakNode[i]==NOT){
            fprintf(fptr1,"G%d,NOT,IN,%d,OUT,%d\n",i,FinalNet[i][0],FinalNet[i][2]); // write the raw of net if gate is NOT
        }
        else if(gatesOfBreakNode[i]==NAND_2 ){
            fprintf(fptr1,"G%d,NAND_2,IN,%d,%d,OUT,%d\n",i,FinalNet[i][0],FinalNet[i][1],FinalNet[i][2]);// write the raw of net if gate is NAND
        }
        else if(gatesOfBreakNode[i]==NOR_2 ){
            fprintf(fptr1,"G%d,NOR_2,IN,%d,%d,OUT,%d\n",i,FinalNet[i][0],FinalNet[i][1],FinalNet[i][2]);// write the raw of net if gate is NOR
        }
        i++;
    }
    return "FileOfGates.txt";
}
...
readLib(char* Lib,int gateLib[],int PMOSConToOut[],int NMOSConToOut[]){
char ch;
   FILE *fp;
   fp = fopen(remove_white_spaces(Lib), "r"); // read modet          ///////////////////////////////////////////
   if (fp == NULL)
   ...
   int counter=0 ;
    while(S=strstr(S,"## GATE")){
        S=S+7;
        if(strcmp(remove_white_spaces(stbetween(' ','\n',S)),"NOT")==0){gateLib[counter]=NOT;}
        if(strcmp(remove_white_spaces(stbetween(' ','\n',S)),"NAND_2")==0){gateLib[counter]=NAND_2;}
        if(strcmp(remove_white_spaces(stbetween(' ','\n',S)),"NOR_2")==0){gateLib[counter]=NOR_2;}
        if(strcmp(remove_white_spaces(stbetween(' ','\n',S)),"XOR_2")==0){gateLib[counter]=XOR_2;}
       ...
       while(ptrNet != NULL)
            ...
            while(ptrNet2 != NULL)
             {
                if(j==1){
                    ...
                    net[i][0]=atoi(ptrNet2);
                    ptrNet2 = strtok(NULL, " ");
                    j++;
                }
                else if(j==4){
                    net[i][1]=atoi(ptrNet2);
                    ptrNet2 = strtok(NULL, " ");
                    j++;
                }
                 else if(j==5){
                    net[i][2]=atoi(ptrNet2);
                    ptrNet2 = strtok(NULL, " ");
                    j++;
                }
             }
        ...
        struct Graph* gr = createDAG(net,trans);
        int j=0;
        while(gr->head[outp-1]->next[j]!=0){
            if(gr->head[outp-1]->rededgeVal[j]==PMOS){
                PMOSConToOut[counter]=PMOSConToOut[counter]+1;
            }
            else if(gr->head[outp-1]->rededgeVal[j]==NMOS){
                NMOSConToOut[counter]=NMOSConToOut[counter]+1;
            }
            j++;
        }
...
///////////////////////////////////////////////oldcode/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    node = initialization(inp,testinp,testout,out,netlist,testvector,logicgate,node,sizeofnode,&Lib,"file1.txt");  ////it stores the information from file in the appropriate array
    ...
    int matchGate[1000][2]={};
    int testInpNet[100]={};
    int OutOfNet[100]={} ;

    FinalNetcount=convertInformation(testout,gNOT,gNOR,gNAND,gXOR,node,netlist,inp,testinp,logicgate,netlistOut,FinalNet,FinalNetcount,transistorNet,inpNet,nodeNet,matchGate,testInpNet,OutOfNet);
...
///////////////////////////////////////////////new code/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     memset(possibleBreak, 0, sizeof(possibleBreak) );
     ...
     FinalNodeNet=initialization(inpNet,testinp,testout,OutOfNet,FinalNet,testvector,transistorNet,FinalNodeNet,sizeofnode,&Lib,nameOfFile[0] );//initializate the new file

     int gateLib[20]={}; // gates there are in library
     int PMOSConToOut[20]={}; // num of conection of PMOS to the output of gates
     int NMOSConToOut[20]={}; // num of conection of NMOS to the output of gates

     readLib(Lib,gateLib,PMOSConToOut,NMOSConToOut); // initialize the structure of every gate's graph (how many PMOS and NMOS are conected to every gate's output)

     struct Graph* grForGates= createDAG(FinalNet,transistorNet);//create the Directed Graph


     findPossibleBreak(possibleBreak,FinalNet); // finde possible breaks for the fisrst file which are the division of the gates
     i=0;
     while(possibleBreak[i]!=0 ){
       i++;
     }
     int tempvar=0;
     while(OutOfNet[tempvar]!=0){  // add the outputs of first file to possible brakes which are the division for the last gates
       possibleBreak[i]=OutOfNet[tempvar];
       i++;
       tempvar++;
     }
     selectionSort(possibleBreak,i-1);

     int gatesOfBreakNode[500]={}; //gates that correspond to each possibleBreak

     i=0;
     while(possibleBreak[i]){ // fill the array gatesOfBreakNode for each possibleBreak//////////////////////////////
        int j=0;
        int numofPMOS=0;
        int numofNMOS=0;
        while(grForGates->head[possibleBreak[i]-1]->rededgeVal[j]!=NULL ){ // find how many PMOS and NMOS is conected to each PossibleBreak Node
            if(grForGates->head[possibleBreak[i]-1]->rededgeVal[j]==PMOS){
                numofPMOS++;
            }
            else if(grForGates->head[possibleBreak[i]-1]->rededgeVal[j]==NMOS){
                numofNMOS++;
            }
            j++;
        }
        j=0;
        while(gateLib[j]!=0){ // fill GatesOfBreakNode array
            if(numofPMOS==PMOSConToOut[j] && numofNMOS==NMOSConToOut[j]){ //check if gate we found is matched with some gate from lib for red edges
               int k=0;
               while(grForGates->head[possibleBreak[i]-1]->nextBlue[k]!=0){  // for every blue edge of gate

                    if(grForGates->head[possibleBreak[i]-1]->blueedgeVal[k]==PMOS){ //check if the gate has as much parallel NMOS as serial PMOS
                        struct GraphNode* n= grForGates->head[possibleBreak[i]-1]->nextBlue[k];
                        int c=0;
                        while(n!=0){ //finde the number of serial PMOS
                            n=n->nextBlue[0];
                            c++;
                        }
                        if(c!=NMOSConToOut[j]){ // check if parallel NMOS is equal to serial PMOS
                            printf("there is not such gate\n");
                            return 0;
                        }
                    }
                    else if(grForGates->head[possibleBreak[i]-1]->blueedgeVal[k]==NMOS){//check if the gate has as much parallel PMOS as serial NMOS
                        struct GraphNode* n= grForGates->head[possibleBreak[i]-1]->nextBlue[k];
                        int c=0;
                        while(n!=0){ //finde the number of serial NMOS
                            n=n->nextBlue[0];
                            c++;
                        }
                        if(c!=PMOSConToOut[j]){ // check if parallel PMOS is equal to serial NMOS
                            printf("there is not such gate\n");
                            return 0;
                        }
                    }
     ...
     int netofgates[500][3]={};// netlist of each gate in array gatesOfBreakNode

     i=0;
     while(possibleBreak[i]!=0){ // for every possibleBreak fill the array netofgates
        int tempArray[2]={};
        int j=0;
        int position=0;
        while(grForGates->head[possibleBreak[i]]->next[j]!=NULL){ // find the inputs of each gate
            if(grForGates->head[possibleBreak[i]]->next[j]->nodenumber!=tempArray[0]){
                tempArray[position] = grForGates->head[possibleBreak[i]]->next[j]->nodenumber;
                position++;
            }
            j++;
        }
        if(gatesOfBreakNode[i]==NAND_2 || gatesOfBreakNode[i]== NOR_2){ // fill the netlist for each gate (netofgates)
            netofgates[i][0]=tempArray[0];
            netofgates[i][1]=tempArray[1];
            netofgates[i][2]=possibleBreak[i];
        }
        ...
        i=0;
        int distinctnode[500]={};
        int countdistinctnode=0;
        while(netofgates[i][0]!=0){ //finds the distinct values of the netofgates
            int k=0;
            while(k<3){
                int check=0;
                int j=0;
                while(distinctnode[j]!=0){
                    if(distinctnode[j]==netofgates[i][k]) {check=1;}
                    j++;
                }
                if(check==0){
                    distinctnode[countdistinctnode]=netofgates[i][k];
                    countdistinctnode++;
                }
                k++;
            }
        ...
        int copynetofgates[500][3]={};
        int newOutOfNet[sizeof(OutOfNet)]={};
        int newinpNet[sizeof(inpNet)]={};
        i=0;
        while (netofgates[i][0]!=0){//copy netofgates
            copynetofgates[i][0]=netofgates[i][0];
            copynetofgates[i][1]=netofgates[i][1];
            copynetofgates[i][2]=netofgates[i][2];
            i++;
        }

        i=0;
        int newval=1;
        while (distinctnode[i]!=0){ //scale down the value of nodes in net of gates
            int j=0;
            while(netofgates[j][0]!=0){ // set the scaled values of new netofgates
                if(copynetofgates[j][0]==distinctnode[i]){
                    netofgates[j][0]=newval;
                }
                if(copynetofgates[j][1]==distinctnode[i]){
                    netofgates[j][1]=newval;
                }
                if(copynetofgates[j][2]==distinctnode[i]){
                    netofgates[j][2]=newval;
                }
                j++;
            }
            j=0;
            while(OutOfNet[j]!=0){  // set the values of new outputs
                if(OutOfNet[j]==distinctnode[i]){newOutOfNet[j]=newval;}
                j++;
            }
            j=0;
            while(inpNet[j]!=0){  // set the values of new inputs
                if(inpNet[j]==distinctnode[i]){newinpNet[j]=newval;}
                j++;
            }
            i++;
            newval++;
        ...
        char* filename=createFileOfGates( netofgates,gatesOfBreakNode,newinpNet ,newOutOfNet,Lib);
          ...
          memset(OutOfNet, 0, sizeof(OutOfNet) );
          sizeofnode=&temp;
          memset(netlistOut, 0, sizeof(netlistOut) );
          FinalNetcount=0;
          memset(matchGate, 0, sizeof(matchGate) );
          memset(testInpNet, 0, sizeof(testInpNet) );
          memset(nodeNet, 0, sizeof(nodeNet) );

          //(practicly the algorithm from lab 3) converting information from file with gates to a flat netlist
          FinalNetcount=convertInformation(newOutOfNet,gNOT,gNOR,gNAND,gXOR,FinalNodeNet,netofgates,newinpNet,newinpNet,gatesOfBreakNode,netlistOut,FinalNet,FinalNetcount,transistorNet,inpNet,nodeNet,matchGate,testInpNet,OutOfNet);
          ...
          int manualtestvector[8][5]={{1,1,1,1,1},{2,1,2,1,1},{2,1,2,1,2},{2,1,2,2,2},{1,1,2,1,2},{1,2,1,1,2},{2,2,2,2,2},{0,0,0,0,0}};//2means1 and 1means0
         ...
         while(manualtestvector[i][0]!=0){ //for every test vector
            ...
            while(OutOfNet[k]!=0){                                                        //////// prints every output we want to see
                ...
                else{printf("the output %d is : %d \n",newOutOfNet[k],newNode[OutOfNet[k]]);}
                k++;
            }
